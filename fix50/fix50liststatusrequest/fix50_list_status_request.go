// Code generated by protofix; DO NOT EDIT.
// Copyright 2021 The Go Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

// Package fix50liststatusrequest is a format of the FIX.5.0 ListStatusRequest message.
package fix50liststatusrequest

import (
	f0 "github.com/protofix/protofix/codecfix"
	"github.com/protofix/protofix/marshfix"
)

var (
	FIX50ListStatusRequestMarshaler   = marshfix.Marshal{Tag: "FIX50", Format: FIX50ListStatusRequest}
	FIX50ListStatusRequestUnmarshaler = marshfix.Unmarshal{Tag: "FIX50", Format: FIX50ListStatusRequest}
)

// FIX50ListStatusRequest is a FIX.5.0 format of the ListStatusRequest message which maps the codecs into individual fields.
var FIX50ListStatusRequest = f0.Format{
	Fields: map[int]f0.Codec{
		ListID66:          f0.Fld{Req, f0.ASCII, f0.String(), f0.Var{1, 65536}},
		Text58:            f0.Fld{Opt, f0.ASCII, f0.String(), f0.Var{1, 65536}},
		EncodedTextLen354: f0.Fld{Opt, f0.ASCII, f0.Length(), f0.Var{1, 19}},
		EncodedText355:    f0.Fld{Opt, f0.ASCII, f0.Bytes(), f0.Var{1, 65536}},
	},
	Unknown0: f0.UnknownFld{f0.ASCII, f0.Unknown(), f0.Var{1, 65536}},
	Sort: []int{
		ListID66,          // STRING
		Text58,            // STRING
		EncodedTextLen354, // LENGTH
		EncodedText355,    // DATA
	},
}

const Req, Opt = true, false

const (
	ListID66          = 66  // STRING
	Text58            = 58  // STRING
	EncodedTextLen354 = 354 // LENGTH
	EncodedText355    = 355 // DATA
)
